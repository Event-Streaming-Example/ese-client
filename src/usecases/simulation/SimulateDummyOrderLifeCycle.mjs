...
import { ORDER_CREATED, ORDER_ALLOCATED, ORDER_STARTED, ORDER_COMPLETED } from '../../entities/Events.mjs';
...
async function createAndPushDummyOrderEvent(orderId, state) {
    const data = { orderId: orderId, state: state, timestamp: Date.now() }
    const ip = await getDeviceIPAddress()
    const eventPayload = createEventPayload(ORDER_STATE_UPDATE_EVENT, ip, data)
    eventCounter += 1
